...
async fn batch_handler(
    ...
    let l1_chain_spec = actor.get_chain_spec(&batch_request.l1_network)?;
    let taiko_chain_spec = actor.get_chain_spec(&batch_request.network)?;
    let mut sub_request_keys = Vec::new();
    let mut sub_request_entities = Vec::new();
    let mut sub_l2_block_numbers = Vec::new();
    for BatchMetadata {
        batch_id,
        l1_inclusion_block_number,
    } in batch_request.batches.iter()
    {
        let l2_block_numbers = parse_l1_batch_proposal_tx_for_pacaya_fork(
            &l1_chain_spec,
            &taiko_chain_spec,
            *l1_inclusion_block_number,
            *batch_id,
        ...
        for l2_block_number in l2_block_numbers {
            let (chain_id, l2_block_hash) =
                get_task_data(&batch_request.network, l2_block_number, actor.chain_specs()).await?;
            ...
            let request_key = SingleProofRequestKey::new(
                ...
                batch_request.prover.clone().to_string(),
            )
            .into();
            let request_entity = SingleProofRequestEntity::new(
                ...
                *l1_inclusion_block_number,
                ...
                batch_request.proof_type.clone(),
            ...
            sub_l2_block_numbers.push(l2_block_number);
        }
    }

    let agg_request_key =
        AggregationRequestKey::new(batch_request.proof_type, sub_l2_block_numbers.clone()).into();
    let agg_request_entity_without_proofs = AggregationRequestEntity::new(
        sub_l2_block_numbers,
        vec![],
        batch_request.proof_type,
